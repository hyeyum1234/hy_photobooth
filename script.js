document.addEventListener("DOMContentLoaded", () => {
  ...
  // ✅ 셔터음 수정: 아이폰 사파리 대응
  // 아이폰은 타이머 안에서 소리 재생이 막힘
  // → 촬영 버튼 누를 때(사용자 터치 순간) 미리 소리를 예약해둠
  let shutterScheduled = false;
  ...
  // 사용자 터치/클릭 순간에 AudioContext 활성화
  ...
  // ✅ 셔터음: 사용자 터치 순간에 AudioContext로 3초 뒤 재생 예약
  function scheduleShutter(delaySeconds) {
    ...
    // AudioContext 내부 시계로 정확한 시간에 재생 (타이머 오차 없음)
  ...
  function updateStickerHint() {
    ...
    hint.textContent = selectedStickerIndex !== null
      ? "✅ 스티커 선택됨! 사진 위 원하는 위치를 탭하세요 / 스티커를 더블탭하면 삭제"
      : "";
  }
  ...
  // ✅ 모바일 더블탭 수정: touchstart로 감지 (click보다 빠르고 정확)
  if (isMobile()) {
    resultCanvas.addEventListener("touchstart", (e) => {
      ...
      const isDoubleTap = (now - lastTapTime < 350) && dist < 40;

      if (isDoubleTap) {
        // 더블탭 → 삭제만 하고 끝
        e.preventDefault(); // 이후 click 이벤트 막기
  ...
  captureBtn.onclick = () => {
    ...
    // ✅ 셔터음: 촬영 버튼 누르는 순간(사용자 터치) AudioContext로 3초 뒤 재생 예약
    // 아이폰은 사용자 제스처 안에서만 소리 허용 → 타이머 밖에서 미리 예약
    scheduleShutter(3);
