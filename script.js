document.addEventListener("DOMContentLoaded", () => {
  ...
  let lastTapX = 0;
  let lastTapY = 0;
  let blockNextClick = false; // ✅ 핵심: 더블탭 후 click 이벤트 차단 플래그
  ...
  async function loadShutterSound() {
    try {
      ...
      console.warn("AudioContext 실패:", e);
    }
  }
  ...
  function scheduleShutter(delaySeconds) {
  ...
  function updateStickerHint() {
    ...
    hint.textContent = selectedStickerIndex !== null
      ? "✅ 스티커 선택됨! 원하는 위치 탭 = 붙이기 / 스티커 위 더블탭 = 삭제"
      : "";
  }
  ...
  if (isMobile()) {
    // ✅ touchstart로 더블탭 감지 → 삭제 후 click 완전 차단
    resultCanvas.addEventListener("touchstart", (e) => {
      const touch = e.touches[0];
      const { canvasX, canvasY } = getCanvasCoords(touch.clientX, touch.clientY);
      const now = Date.now();
      ...
      if (isDoubleTap) {
        e.preventDefault(); // 터치 이후 발생하는 click 이벤트 차단
        blockNextClick = true; // 추가 보험용 플래그
      ...
      lastTapX = canvasX;
      lastTapY = canvasY;
    }, { passive: false }); // passive: false 여야 preventDefault() 가 작동함

    // 모바일 싱글탭: 스티커 배치
    resultCanvas.addEventListener("click", (e) => {
      // ✅ 더블탭 후 발생한 click이면 완전 무시
      if (blockNextClick) {
  ...
  captureBtn.onclick = () => {
    ...
    countTimer = setInterval(() => {
      ...
      if (count <= 0) {
        ...
        captureCtx.translate(PHOTO_W, 0);
        captureCtx.scale(-1, 1);
        captureCtx.drawImage(video, 0, 0, PHOTO_W, PHOTO_H);
  ...
  function redraw() {
    ...
    for (let i = 0; i < TOTAL; i++) {
      ...
      if (photoImages[i]) {
        ...
        ctx.save();
        ctx.translate(dx + PHOTO_W, dy);
        ctx.scale(-1, 1);
        ctx.drawImage(video, 0, 0, PHOTO_W, PHOTO_H);
        ctx.restore();
        if (count > 0) {
